...
//Sendy subscribe functions
//Shortcode for rendering the form - still need #lf_subscribe_btn button added to the HTML
function tao_subscribe($atts) {
	wp_enqueue_script( 'tao-sendy', plugin_dir_url(__FILE__) . 'js/subscribe.min.js', 'jquery', '1.0.0', false);	
	//Get the correct list ID
	$list_id = TAO_SENDY_LIST;
    $lid = isset($atts['list']) ? $atts['list'] : '';
    $id = isset($atts['id']) ? $atts['id'] : '1';
    $popup = isset($atts['popup']) ? $atts['popup'] : '';
    $text = isset($atts['text']) ? $atts['text'] : 'Subscribe';
	switch ($lid) {
		case '1':
			$list_id = TAO_SENDY_LIST;
			break;
	...
	$nonce = wp_create_nonce('tao_subscribe');	
	$html = '<div class="tao_subscribe tao_subscribe_' . $id . ' ' . $class . '">';
	//Thank you
	$html .= '<div class="tao_sub_thx_row tao_sub_thx_row_' . $id . '" style="display:none">';
	$html .= '<div class="tao_thx_img_div"></div>';
	$html .= '<div style="line-height:23px" id="tao_sub_thx_message">Thank you for registering your interest.</div>';
	$html .= '</div>';
	//Error field
	$html .= '<div class="tao_sub_err_row tao_sub_err_row_' . $id . '" style="display:none">';
	$html .= '<div class="tao_err_img_div"></div>';
	$html .= '<div style="line-height:23px" id="tao_sub_err_message_' . $id . '"></div>';
	$html .= '</div>';	
    //Form	
	$html .= '<div class="tao_sub_frm tao_sub_frm_' . $id . '"><label>First name<br><input type="text" class="tao_input" id="tao_name_' . $id . '" placeholder="First Name" autocomplete="first-name"></label></div>';
    $html .= '<div class="tao_sub_frm tao_sub_frm_' . $id . '"><label>Email<br><input type="email" class="tao_input" id="tao_email_' . $id . '" placeholder="Email" autocomplete="email"></label></div>';
	$html .= '<div class="tao_sub_frm tao_sub_btn_row tao_sub_btn_row_' . $id . '"><a class="tao_btn tao_btn_red tao_submit" data-id="' . $id . '" id="tao_submit_' . $id . '" data-id="' . $list_id . '" data-popup="' . $popup . '">' . $text . '</a></div>';
	$html .= '</div>';
	$html .= '<script>window.tao = {sbn: "' . $nonce . '", s: "' . admin_url('admin-ajax.php') . '"};</script>';
	return $html;	
}
add_shortcode( 'tao_subscribe', 'tao_subscribe');


//Subscribe with Sendy locally
function tao_ajax_subscribe() {
	//Secure the connection
	check_ajax_referer( 'tao_subscribe', 'nc' );	
	$ip = tao_get_client_ip();
	$location = tao_get_country($ip);	
	$args = array(
		'method' => 'POST',
		'headers'  => array(
		...
		'body' => array(		
			...
			'gdpr' => (tao_gdpr_zone($location) == true) ? "true" : "false",
			'list' => isset($_POST['list']) ? $_POST['list'] : TAO_SENDY_LIST,
			'boolean' => "true"	
		)	
	);	
	//Submit subscription
...
add_action( 'wp_ajax_nopriv_tao_subscribe', 'tao_ajax_subscribe');
add_action( 'wp_ajax_tao_subscribe', 'tao_ajax_subscribe');

//Is this location in a GDPR zone?
function tao_gdpr_zone($location) {
	if (in_array($location->country->isoCode, TAO_GDPR_ZONES)) {
		return true;
	} else {
		return false;
	}
...
function tao_get_country($ip) {	
	$geo = new Reader(MAXMIND_LOCATION);
	return $geo->city($ip);
}
